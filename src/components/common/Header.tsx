...
export const Header: React.FC<HeaderProps> = ({ currentTab = 'dashboard', onOpenChapterManager, onOpenAuthModal }) => {
  const { isScanning, scanProgress, scanMistakesWithGemini, mistakes, clearAllData } = useAcademic();
  const { user, publicProfile, signOut, authError, clearAuthError } = useAuth();
  ...
  const getTabTitle = (tab: string) => {
    switch (tab) {
      ...
      case 'chat':
        return 'Study Chat & Peer Discussions';
      case 'questionbank':
        return 'Question Bank & Collaborative Library';
      case 'calendar':
        return 'Academic Calendar & Milestones';
      case 'insights':
        return 'Gemini Weakness Diagnostics & Patterns';
      case 'settings':
        return 'System Settings & Gemini BYOK';
      default:
        return 'RECALL';
    }
  };
    ...
    <header className="h-14 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 px-6 flex items-center justify-between sticky top-0 z-10 select-none transition-colors">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            <div className="w-5 h-5 rounded-full bg-indigo-600 dark:bg-indigo-500 text-white flex items-center justify-center text-[10px] font-mono font-bold">
            ...
            <div className="absolute right-0 mt-2 w-64 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl py-2 z-50 animate-in fade-in zoom-in-95">
              <div className="px-4 py-2 border-b border-zinc-100 dark:border-zinc-800">
                <p className="text-xs font-bold text-zinc-900 dark:text-zinc-100 font-sans">
                  {isGuest ? 'Guest Student' : (user?.displayName || user?.username)}
                </p>
                <p className="text-[11px] text-zinc-500 dark:text-zinc-400 font-mono truncate">
                  {isGuest ? 'Local cloud session' : `@${user?.username}`}
                </p>
              </div>
                  ...
                  <div className="space-y-1">
                    {onOpenAuthModal && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold font-mono text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-colors shadow-sm"
                      >
